...
HOST_FRAMEWORKS="-framework AppKit -framework Foundation -framework UserNotifications"
...
build_module() {
  ...
  if [ "$MODE" = "asan" ]; then
    if [ ! -x "$HOST" ]; then
      echo "[hw_calendar] build the ASan host before its module" >&2
      return 1
    fi
    asan_runtime=$(otool -L "$HOST" | awk '/libclang_rt\.asan_osx_dynamic\.dylib/ {print $1; exit}')
    if [ -z "$asan_runtime" ] || [ ! -f "$asan_runtime" ]; then
      echo "[hw_calendar] could not resolve the host ASan runtime" >&2
      return 1
    fi
  fi
...
printf '[hw_calendar] built hot-reload %s %s\n' "$MODE" "$PART"
